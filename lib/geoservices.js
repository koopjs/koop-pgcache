...
Geoservices.parse = function (options) {
  var select = options.simplify ? stubSimplified(options) : stubStandard(options)
  if (options.where) select += ' WHERE ' + Geoservices.parseWhere(options.where)
  select += addGeometry(options)
  select += addOrder(options)
