...
var Indexing = {
   ...
   */
  addIndexes: function (table, options, callback) {
    var info = {}
    if (options.fields) {
    ...
    var indexes = this.prepareIndexes(info)
    this._addIndexes(table, indexes, callback)
  },
  ...
  prepareIndexes: function (info) {
    ...
    var indexes = []
    if (info._indexGeohash) indexes.concat(geohashindexes)
    if (info._indexGeometry) indexes.push(geometryIndex)
    if (info && info.fields && info._indexFields) indexes.concat(prepareFieldindexes(info.fields))
    return indexes
  },
   ...
   */
  _addIndexes: function (table, indexes, callback) {
  ...
  // Note: the query function is replaced at runtime when the connection is made to the database
  query: null
}
...
function createIndex (table, name, using, callback) {
  ...
  Indexing.query(sql, function (err) {
